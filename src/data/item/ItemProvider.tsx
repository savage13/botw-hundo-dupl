...
import { Item, ItemStack, ItemType } from "./type";

import Images from "assets/img";
import { deprecatedRegister } from "./legacy";

/*
 * Load items from items.yaml files and registers them in memory
 */
type ItemIdMap = { [id: string]: Item};
...
export const ItemProvider: React.FC<PropsWithChildren> = ({children}) => {
    ...
    const [getItem, searchItem] = useMemo(()=>{
        ...
        if(itemIdMap && itemSearchMap){
            ...
            const searchItem = (word: string): ItemStack | undefined => {
                return searchItemMemoized(word, itemIdMap, itemSearchMap);
            };
    ...
    return (
        <ItemContext.Provider value={{getItem, searchItem}}>
            {children}
...
const loadItemDataAsync = async ():Promise<[ItemIdMap, ItemSearchMap]> => {
    ...
    // temporary
    Object.values(idMap).forEach(item=>deprecatedRegister(item));
...
const registerItem = (idAndSearch: string, option: ItemOption, type: ItemType, outIdMap: ItemIdMap, outSearchMap: ItemSearchMap) => {
    ...
    const animatedImage = option.animated ? Images[id+"Animated"] : undefined;
    const item = new ItemImpl(id, type, option.repeatable ?? false, option.stackable ?? false, image, animatedImage);
    outIdMap[id] = item;
    if(search){
...
const splitIdAndSearch = (idAndSearch: string): [string, string|undefined] => {
    const i = idAndSearch.indexOf(":");
    if(i<0){
        return [idAndSearch, undefined];
    }else{
        return [idAndSearch.substring(0, i), idAndSearch.substring(i+1).toLowerCase()];
    }
};

const searchItemMemoized = (name: string, idMap: ItemIdMap, searchMap: ItemSearchMap): ItemStack | undefined => {
    if (name in MemoizedSearchResults){
        return MemoizedSearchResults[name];
    }

    // legacy search must happen first to make sure legacy items are matched exactly
    const legacyItem = searchLegacyItemNames(name);
    if(legacyItem){
        MemoizedSearchResults[name] = legacyItem;
        return legacyItem;
    }

    const result = searchItemInMap(name, idMap, searchMap);
    MemoizedSearchResults[name] = result;
    return result;
}

const searchLegacyItemNames = (name: string): ItemStack | undefined => {
    return undefined;
}
